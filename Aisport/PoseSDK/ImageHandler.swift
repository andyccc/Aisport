...
class ImageHandler {
  ...
  private func preprocess(of pixelBuffer: CVPixelBuffer) -> Data? {
    ...
    guard let thumbnail = pixelBuffer.resize(from: CGRect(origin: CGPoint(x: 0, y: 0), size: pixelBuffer.size), to: modelSize)
  ...
  private func postprocess() -> KPResult? {
    ...
    var result = KPResult(dots: [], score: totalScore)
    ...
    for (index, part) in BodyPart.allCases.enumerated() {
      let position = CGPoint(
        x: CGFloat(coords[index].x),
        y: CGFloat(coords[index].y)
      )
