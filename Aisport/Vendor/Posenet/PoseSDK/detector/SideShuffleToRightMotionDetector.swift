//
//  SideShuffleToRightMotionDetector.swift
//  Aisport
//
...
class SideShuffleToRightMotionDetector:BaseMotionDetector{
    ...
    override init(maxCycleSeconds:Double) {
        ...
        motion=MotionEnum.SideShuffleToRight
    }
    ...
    override func detect(kpResList:[KPResult])->MotionMsg?{
        ...
        if state==SideShuffleToRightMotionDetector.STATE_END{
    ...
    func updateState(kpResList:[KPResult]){
        if kpResList.count<4{
            state=SideShuffleToRightMotionDetector.STATE_NOT_DOING
            return
        }
        ...
        if state==SideShuffleToRightMotionDetector.STATE_START{
            state = (curPos.x-prePos.x<0) ? SideShuffleToRightMotionDetector.STATE_DOING : SideShuffleToRightMotionDetector.STATE_END
        }
        else if state==SideShuffleToRightMotionDetector.STATE_DOING{
            state = (curPos.x-prePos.x<0) ? SideShuffleToRightMotionDetector.STATE_DOING : SideShuffleToRightMotionDetector.STATE_END
        }
        else if state==SideShuffleToRightMotionDetector.STATE_END{
            state = SideShuffleToRightMotionDetector.STATE_NOT_DOING
        }
        else if state==SideShuffleToRightMotionDetector.STATE_NOT_DOING{
            state = (curPos.x-prePos.x < -40) ? SideShuffleToRightMotionDetector.STATE_START : SideShuffleToRightMotionDetector.STATE_NOT_DOING
        }
