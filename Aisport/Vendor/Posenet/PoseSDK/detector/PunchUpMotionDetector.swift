...
class PunchUpMotionDetector:BaseMotionDetector{
    ...
    func updateState(kpResList:[KPResult]){
        ...
        if state==PunchUpMotionDetector.STATE_START{
            state = (curWrist.y-preWrist.y<0) ? PunchUpMotionDetector.STATE_DOING : PunchUpMotionDetector.STATE_END
        }
            ...
            state = (curWrist.y-preWrist.y < -40) ? PunchUpMotionDetector.STATE_START : PunchUpMotionDetector.STATE_NOT_DOING
        }
