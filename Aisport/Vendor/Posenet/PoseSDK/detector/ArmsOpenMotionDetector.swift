...
import Foundation
class ArmsOpenMotionDetector:BaseMotionDetector{
    ...
    override func detect(kpResList:[KPResult])->MotionMsg?{
        ...
        if state==ArmsOpenMotionDetector.STATE_END{
    ...
    func updateState(kpResList:[KPResult]){
        if kpResList.count<4{
            state=ArmsOpenMotionDetector.STATE_NOT_DOING
            return
        }
        ...
        if state==ArmsOpenMotionDetector.STATE_START{
            state = (curPos.x-prePos.x<0) ? ArmsOpenMotionDetector.STATE_DOING : ArmsOpenMotionDetector.STATE_END
        }
        else if state==ArmsOpenMotionDetector.STATE_DOING{
            state = (curPos.x-prePos.x<0) ? ArmsOpenMotionDetector.STATE_DOING : ArmsOpenMotionDetector.STATE_END
        }
        else if state==ArmsOpenMotionDetector.STATE_END{
            state = ArmsOpenMotionDetector.STATE_NOT_DOING
        }
        else if state==ArmsOpenMotionDetector.STATE_NOT_DOING{
            state = (curPos.x-prePos.x < -40) ? ArmsOpenMotionDetector.STATE_START : ArmsOpenMotionDetector.STATE_NOT_DOING
        }
