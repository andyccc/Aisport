//
//  SideShuffleToLeftMotionDetector.swift
//  Aisport
//
...
class SideShuffleToLeftMotionDetector:BaseMotionDetector{
    ...
    override init(maxCycleSeconds:Double) {
        ...
        motion=MotionEnum.SideShuffleToLeft
    }
    ...
    override func detect(kpResList:[KPResult])->MotionMsg?{
        ...
        if state==SideShuffleToLeftMotionDetector.STATE_END{
    ...
    func analyse(kpResList:[KPResult])->MotionMsg?{
        ...
        let wirstXList=newKpList.map{(kpResult)->Float in
            return Float(kpResult.dots[KP.RIGHT_WRIST.rawValue]!.x)
        }
        ...
        //判断右手的变动幅度
        if maxX-minX<100{
            return MotionMsg(motion: motion, status: MotionStatus.bad, msg:"幅度不足")
        }
    ...
    func updateState(kpResList:[KPResult]){
        if kpResList.count<4{
            state=SideShuffleToLeftMotionDetector.STATE_NOT_DOING
            return
        }
        ...
        guard let curPos=curRes.dots[KP.RIGHT_WRIST.rawValue],let prePos=preRes.dots[KP.RIGHT_WRIST.rawValue] else {
            return
        }
        
        if state==SideShuffleToLeftMotionDetector.STATE_START{
            state = (curPos.x-prePos.x>0) ? SideShuffleToLeftMotionDetector.STATE_DOING : SideShuffleToLeftMotionDetector.STATE_END
        }
        else if state==SideShuffleToLeftMotionDetector.STATE_DOING{
            state = (curPos.x-prePos.x>0) ? SideShuffleToLeftMotionDetector.STATE_DOING : SideShuffleToLeftMotionDetector.STATE_END
        }
        else if state==SideShuffleToLeftMotionDetector.STATE_END{
            state = SideShuffleToLeftMotionDetector.STATE_NOT_DOING
        }
        else if state==SideShuffleToLeftMotionDetector.STATE_NOT_DOING{
            state = (curPos.x-prePos.x > 40) ? SideShuffleToLeftMotionDetector.STATE_START : SideShuffleToLeftMotionDetector.STATE_NOT_DOING
        }
