//
//  StepRightOutMotionDetector.swift
//  Aisport
//
...
class StepRightOutMotionDetector:BaseMotionDetector{
    ...
    override init(maxCycleSeconds:Double) {
        ...
        motion=MotionEnum.StepRightOut
    }
    ...
    override func detect(kpResList:[KPResult])->MotionMsg?{
        ...
        if state==StepRightOutMotionDetector.STATE_END{
    ...
    func updateState(kpResList:[KPResult]){
        if kpResList.count<4{
            state=StepRightOutMotionDetector.STATE_NOT_DOING
            return
        }
        ...
        if state==StepRightOutMotionDetector.STATE_START{
            state = (curAnkle.x-preAnkle.x<0) ? StepRightOutMotionDetector.STATE_DOING : StepRightOutMotionDetector.STATE_END
        }
        else if state==StepRightOutMotionDetector.STATE_DOING{
            state = (curAnkle.x-preAnkle.x<0) ? StepRightOutMotionDetector.STATE_DOING : StepRightOutMotionDetector.STATE_END
        }
        else if state==StepRightOutMotionDetector.STATE_END{
            state = StepRightOutMotionDetector.STATE_NOT_DOING
        }
        else if state==StepRightOutMotionDetector.STATE_NOT_DOING{
            state = (curAnkle.x-preAnkle.x < -40) ? StepRightOutMotionDetector.STATE_START : StepRightOutMotionDetector.STATE_NOT_DOING
        }
