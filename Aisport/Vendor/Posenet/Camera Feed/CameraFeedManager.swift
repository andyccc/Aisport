...
class CameraFeedManager: NSObject {
  ...
  // MARK: Initializer
    @objc(initWithPreviewView:isFront:)
  ...
  private func addVideoDeviceInput() -> Bool {
    ...
    guard
        let camera = AVCaptureDevice.default(.builtInWideAngleCamera, for: .video, position: self.isFront ?? false ? .front : .back)
      else {
...
//    func rotateVideo2(interfaceOrientation: UIInterfaceOrientation)  {
//        session.beginConfiguration()
//        self.previewView.previewLayer.connection?.videoOrientation = interfaceOrientation
//        videoDataOutput.connection(with: .video)?.videoOrientation = interfaceOrientation
//        
//        session.commitConfiguration()
//    }
    
    @objc
    func rotateVideo(interfaceOrientation: UIInterfaceOrientation)  {
        ...
        switch interfaceOrientation {
            case UIInterfaceOrientation.portrait:
                self.adjustAVOutputDataOrientation(aOrientation: AVCaptureVideoOrientation.portrait, layer: layer)
                
                break
                
            case UIInterfaceOrientation.portraitUpsideDown:
                self.adjustAVOutputDataOrientation(aOrientation: AVCaptureVideoOrientation.portraitUpsideDown, layer: layer)
                
                break
                
            case UIInterfaceOrientation.landscapeLeft:
                self.adjustAVOutputDataOrientation(aOrientation: AVCaptureVideoOrientation.landscapeLeft, layer: layer)
                
                
                break
                
            case UIInterfaceOrientation.landscapeRight:
                self.adjustAVOutputDataOrientation(aOrientation: AVCaptureVideoOrientation.landscapeRight, layer: layer)
    ...
    func adjustAVOutputDataOrientation(aOrientation: AVCaptureVideoOrientation, layer: AVCaptureVideoPreviewLayer) {
        layer.connection?.videoOrientation = aOrientation

        for connection:AVCaptureConnection in videoDataOutput.connections {
            for port:AVCaptureInput.Port in connection.inputPorts {
                if port.mediaType == AVMediaType.video {
                    if connection.isVideoOrientationSupported {
                        connection.videoOrientation = aOrientation
                    }
                }
            }
        }
