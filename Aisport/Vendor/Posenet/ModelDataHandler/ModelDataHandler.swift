...
@objc public class ModelDataHandler: NSObject {
    ...
    -> (Result, Times)?
  {
    ...
    @objc
    func oc_runPoseNet(on pixelbuffer: CVPixelBuffer, from source: CGRect, to dest: CGSize) -> ResultData? {
        guard let (result, times) = self.runPoseNet(on: pixelbuffer, from: source, to: dest) ?? (nil, nil) else {
            return nil
        }
  ...
  private func preprocess(of pixelBuffer: CVPixelBuffer, from targetSquare: CGRect) -> Data? {
    ...
    guard let thumbnail = pixelBuffer.resize(from: targetSquare, to: modelSize, type: self.type)
    else {
      return nil
    }
  ...
  private func postprocess(to viewSize: CGSize) -> Result? {
    ...
    do {
        try result.lines = BodyPart.lines.map { map throws -> LineData in
        ...
        guard let to = bodyPartToDotMap[map.to] else {
          throw PostprocessError.missingBodyPart(of: map.to)
            ...
            return LineData(from: Point(x: Float(from.x), y: Float(from.y)), to: Point(x: Float(to.x), y: Float(to.y)))
      }
